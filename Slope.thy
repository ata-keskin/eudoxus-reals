...
proof (rule ccontr, clarsimp, goal_cases)
  ...
  have bounded: "\<forall>z\<in>A. \<bar>?f z\<bar> \<le> C" using 1 by fastforce
  moreover have "\<forall>z\<in>UNIV - A. \<bar>?f z\<bar> \<le> C" using 1 by fastforce
  ultimately have "bounded ?f" by (blast intro: boundedI)
  thus False using bounded_iff_finite_range assms by force
qed

lemma bounded_anti_symmetric:
  assumes "\<forall>z. z < 0 \<longrightarrow> (f :: int \<Rightarrow> int) z = -f (-z)" "\<exists>C. \<forall>n \<ge> 0.\<bar>f n\<bar> \<le> C"
  shows "bounded f"
proof -
  obtain C where "\<forall>n \<ge> 0. \<bar>f n\<bar> \<le> C" using assms by blast
  hence "\<bar>f n\<bar> \<le> C" for n using assms by (cases "0 \<le> n") fastforce+
  thus ?thesis by (rule boundedI)
qed

lemma finite_nonneg_bounded:
  assumes "\<forall>n\<ge>N. (f :: int \<Rightarrow> int) n \<le> C"
  shows "\<exists>C. \<forall>n\<ge>0. f n \<le> C"
proof -
  define S where "S = (\<Union>n\<in>{0..N}. {f n})"
  have bound: "n \<in> {N..} \<Longrightarrow> f n \<le> C" for n using assms by blast
  have "bdd_above S" unfolding S_def by auto
  hence "n \<in> {0..<N} \<Longrightarrow> f n \<le> Sup S" for n by (intro cSup_upper2[OF _ order.refl _]) (fastforce simp: S_def)
  hence "n \<in> {0..} \<Longrightarrow> f n \<le> max C (Sup S)" for n by (metis bound atLeastLessThan_iff atLeast_iff leI max.coboundedI1 max.coboundedI2)
...
lemma slope_positive_lower_bound:
  assumes "slope f" "infinite (f ` {0..} \<inter> {0<..})"
  shows "\<forall>D>0. \<exists>M>0. \<forall>m>0. (m + 1) * D \<le> f (m * M)"
proof -
  {
    fix D :: int assume D_pos: "D > 0"
    ...
    obtain f_M where "2 * (C + D) \<le> \<bar>f_M\<bar>" "f_M \<in> (f ` {0..} \<inter> {0<..})" using int_set_infiniteD[OF assms(2)] mult_left_mono[of "C + D" _ 2] C_nonneg D_nonneg by (meson abs_ge_zero abs_le_D1)
    ...
    moreover have "M \<noteq> 0" using M_bound add1_zle_eq D_pos neg_bound by force
...
proof (cases "N \<ge> 0")
  ...
  hence b1: "\<forall>n\<ge>0.\<forall>m\<ge>N. f n m \<le> C" and b2: "\<forall>n\<ge>N.\<forall>m\<ge>0. f n m \<le> C" using assms True by simp+
  hence "\<forall>n\<in>{0..<N}. \<forall>m\<ge>N. f n m \<le> max C (Sup S)" by fastforce
  hence "\<forall>n\<in>{0..<N}. \<forall>m\<ge>0. f n m \<le> max C (Sup S)" using b0 by (meson atLeastLessThan_iff linorder_not_le)
  moreover have "\<forall>n\<ge>N.\<forall>m\<ge>0. f n m \<le> max C (Sup S)" using b2 by fastforce
